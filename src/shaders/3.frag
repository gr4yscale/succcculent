...
void main() {
  float color = cnoise2(vUv.yx * (abs(cos(iGlobalTime * 0.5)) * 2.) + 50.) * 2.0;
...
gl_FragColor = vec4( base.y + 0.75 + (shading.y * 0.2) - (trippyPattern.x * 0.9), 0.7 + (-shading.z * 0.4) - (trippyPattern.y * 0.4), 0.8 + (trippyPattern.y * 0.5), 1.0);
