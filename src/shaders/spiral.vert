...
void main() {
  ...
  gl_Position = projectionMatrix * modelViewMatrix * vec4( position.x + sin(iGlobalTime + (vUv.y * 2.)) * 0.05, sin(iGlobalTime) * position.y, position.z + cos(iGlobalTime + (vUv.y * 1.)) * 0.05, 1.0 );
